...
instance SymbolTreeMaker Prg where
	symbolize (Prg mainclass classdecllist) = Program classes where
		classes =  (symbolize mainclass) : [symbolize x | x<-classdecllist]

instance SymbolTreeMaker MainClass where
	symbolize (MainClass name args _) = Class name vars methods where
		vars = [] 
		methods = [main] 
		main = Method "main" VoidType [(Var args StringArrayType)] []

instance SymbolTreeMaker ClassDeclaration where
	symbolize (ClassDeclaration name parent varlist methodlist) = Class name vars methods where
...
instance SymbolTreeMaker MethodDeclaration where
	symbolize (MethodDeclaration typ name parlist varlist _ _) = Method name typ parameter vars where
		parameter = [Var name typ  | (typ, name)<-parlist]
		vars = [symbolize x | x<-varlist]
